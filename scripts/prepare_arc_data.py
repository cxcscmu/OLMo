"""
Script for preparing the ARC-Easy data for fine-tuning an OLMo model.
"""
...
def main(opts) -> None:
    ...
    dataset = ds.load_dataset("ai2_arc", "ARC-Easy", split="train")
    print("ARC-Easy train dataset size:", len(dataset))

    log.info("Tokenizing dataset...")
    dataset = dataset.map(
        partial(preprocess, tokenizer=tokenizer, max_seq_len=opts.seq_len),
        batched=False,
        remove_columns=["id", "question", "choices", "answerKey"],
...
def preprocess(example, tokenizer: Tokenizer, max_seq_len: int):
    ...
    answer_key = num_to_letter.get(example["answerKey"], example["answerKey"])

    # Get the correct answer text
    choices = example["choices"]
    answer_idx = ["A", "B", "C", "D", "E"].index(answer_key)
    answer_text = choices["text"][answer_idx]

    # Format: "Question: {question}\nAnswer:" (masked) + " {answer}" (not masked)
    query = "Question: " + example["question"] + "\nAnswer:"

    # Tokenize question part (masked - not trained on)
...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Prepare ARC-Easy dataset")
...
# Example usage:
# python scripts/prepare_arc_data.py /path/to/output/arc_easy \
